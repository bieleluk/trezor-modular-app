...
pub fn build(args: BuildArgs) -> Result<()> {
    ...
    if !args.emulator && !args.debug {
        ...
        postbuild::publish_artifact(&min, &args.project, args.model, args.emulator)?;
    } else {
        run_cargo_subcommand("size", &args, Some(&["-A"]))?;
        run_cargo_subcommand("size", &args, Some(&["-B"]))?;
        postbuild::publish_artifact(&orig, &args.project, args.model, args.emulator)?;
    };
...
pub fn clean() -> Result<()> {
    let mut cmd = process::Command::new("cargo");
    cmd.arg("clean");
    cmd.current_dir(helpers::workspace_dir()?);
