use crate::{args::PyStyleArgs, helpers};
use anyhow::{Result, ensure};
use std::process::Command;

pub fn run(args: PyStyleArgs, check_only: bool) -> Result<()> {
    let ws_dir = helpers::workspace_dir()?;
    let project_dir = ws_dir.join(&args.project);
