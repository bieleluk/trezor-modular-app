...
def call_ext(
    ...
    *,
    ...
    # Serialize to bytes
    buf = io.BytesIO()
    protobuf.dump_message(buf, msg_data)  # type: ignore[reportAttributeAccessIssue]
    ...
    with session:
        ...
        expect_ids = [message_id(cls) for cls in expect]  # type: ignore[reportAttributeAccessIssue]
        try:
            # Find the index of the matching message ID
            idx = expect_ids.index(resp.message_id)

            return protobuf.load_message(buf, expect[idx])  # type: ignore[reportAttributeAccessIssue]
        except Exception:
...
def get_public_node(
    ...
    return call_ext(
        ...
        expect=[funnycoin_messages.PublicKey],  # type: ignore[reportAttributeAccessIssue]
    )
